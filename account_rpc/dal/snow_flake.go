...
var snowFlakeCli *SnowFlake

func InitSonwFlake() {
	snowFlakeCli = NewSnowFlake()
}
...
func NewSnowFlake() *SnowFlake {
	...
	return &SnowFlake{
		...
		workerid:     rd.Int63(),
		datacenterid: rd.Int63(),
		sequence:     rd.Int63(),
	}
}

func GenAccessToken(ctx context.Context) int64 {
	return snowFlakeCli.NextVal(ctx)
}
...
func (s *SnowFlake) NextVal(ctx context.Context) int64 {
	...
	if t > timestampMax {
		s.Unlock()
		klog.CtxErrorf(ctx, "epoch must be between 0 and %d", timestampMax-1)
		return 0
	}
	s.timestamp = now
